...
import frappe
from changai.changai.erp_assistant import extractor, llm
...
def rebuild(cards):
    ...
    # tolerate_failures: a rate-limited/timed-out batch partway through a
    # ~85-doctype build shouldn't throw away every card embedded before it
    # — keep whatever succeeded and skip the rest, rather than aborting
    # with an empty index (see llm.embed's docstring for why this is only
    # safe here, not for live query-time embedding).
    vectors = llm.embed(texts, tolerate_failures=True)  # -> list[list[float] | None]
    kept_vectors, kept_metas = [], []
    for vec, meta in zip(vectors, metas):
        if vec is not None:
            kept_vectors.append(vec)
            kept_metas.append(meta)
    skipped = len(metas) - len(kept_metas)

    if not kept_vectors:
        raise RuntimeError("All embeddings failed — nothing to index.")
    ...
    with open(_meta_path(), "w") as f:
        json.dump(kept_metas, f)
    if skipped:
        try:
            frappe.log_error(
                f"{skipped} of {len(metas)} cards failed to embed and were "
                f"skipped this build — retry later to fill them in.",
                "erp_assistant.rag.rebuild_partial"
            )
        except Exception:
            pass  # the index write above already succeeded; a stale DB
            # connection here shouldn't turn that success into a raised error
